...
/// Linux implementation of [FlutterCropCameraPlatform].
///
/// Camera preview and capture use a native GStreamer/V4L2 method channel
/// ([FlutterCropCameraPlugin]) which registers a [FlutterTexture].
///
/// Gallery picking is handled entirely in Dart via `package:file_selector`
/// (GTK file chooser) so no native code is needed for that.
///
...
class FlutterCropCameraLinux extends FlutterCropCameraPlatform {
  ...
  /// Method channel to the native C++ [FlutterCropCameraPlugin] registered
  ...
  // ── Camera (native GStreamer/V4L2 via method channel) ──────────────────────
  ...
  /// Zoom is not broadly supported on Linux V4L2 devices — silently ignored.
  @override
  Future<void> setZoom(double zoom) async {}

  /// Flash is not broadly supported on Linux V4L2 devices — silently ignored.
  @override
  ...
  Future<String?> takePicture() async {
    try {
      ...
      throw PlatformException(
        code: e.code,
        message: 'Linux takePicture error: ${e.message}',
        details: e.details,
      );
    }
  }

  // ── Gallery (pure Dart — GTK file chooser via file_selector) ──────────────
