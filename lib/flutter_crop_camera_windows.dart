...
///
/// Camera preview and capture use a native C++/WinRT method channel
...
/// (IFileOpenDialog) so no native code is needed for that.
///
...
class FlutterCropCameraWindows extends FlutterCropCameraPlatform {
  ...
  // ── Camera (native C++/WinRT via method channel) ───────────────────────────
  ...
  Future<int?> startCamera({
    ...
    try {
      ...
      throw PlatformException(
        code: e.code,
        message: 'Windows camera error: ${e.message}',
        details: e.details,
      );
    }
  }
  ...
  // ── Gallery (pure Dart — IFileOpenDialog via file_selector) ───────────────
